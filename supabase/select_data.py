...
def get_brains_per_workspace(supabase: Client, workspace_id: Any) -> List[Dict[str, Any]]:
    """
...
def filter_unique_items(items: List[Dict[str, Any]], unique_key: str) -> List[Dict[str, Any]]:
    """
    ...
    if duplicates_found:
        print(f"Note: Duplicate entries found based on '{unique_key}' have been removed.")
    return unique_items
...
def get_selection(num_items: int) -> Optional[List[int]]:
    ...
    """
    while True:
        selection = input("Enter the numbers of your selections, separated by commas (or '0' to stop): ")
        ...
        try:
            ...
            if all(1 <= idx <= num_items for idx in selected_indices):
                ...
                print(f"Please enter numbers between 1 and {num_items}, or '0' to stop.")
        except ValueError:
            print("Invalid input. Please enter numbers separated by commas, or '0' to stop.")
...
def select():
    ...
    if selected_document_indices is None:
        print("Selection stopped. Returning selected workspaces, brains, and documents.")
        return selections
